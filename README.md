...
The range of "E", "EE", "EEE" etc. must satisfy and not overflow the
MAX_BIGINT=9223372036854775807 and MAX_SAFE_INTEGER=9007199254740991 decimal
prefixes. The library validates that at the installation time. Examples:
...
**WARNING:** the above numbers (with `000000` suffixes) are given only as
...
they do, it's also fine). Then save them in `pg-id.config.sql` file as explained
above.
...
By default, the function uses `id_seq` sequence in the function's schema to get
...
`string` type for it (JavaScript's `number` doesn't have enough precision). By
default, node-postgres (and other) libraries return `bigint` as a `string`, exactly
as needed here.

## id_gen_max_safe_integer([seq_name regclass])

Similar to `id_gen()`, but produces only 16 decimal digits (all ids are less
than `Number.MAX_SAFE_INTEGER`). You can assign such an id to a JavaScript
`number` variable and not lose any digits.
...
By default, the function uses `id_seq_timestampic` sequence to get the
...
The function always utilizes the full bigint range (19 decimal digits). There is
no MAX_SAFE_INTEGER-safe version, since we need at least 9 digits for the
timestamp part alone.
...
you store such an id in a JavaScript variable, make sure that you're using
`string` type for it (JavaScript's `number` doesn't have enough precision). By
default, node-postgres (and other) libraries return `bigint` as a `string`,
exactly as needed here.

## id_gen_monotonic_max_safe_integer([seq_name])

Similar to `id_gen_monotonic()`, but produces only 16 decimal digits (all ids
are less than `Number.MAX_SAFE_INTEGER`). You can assign such an id to a
JavaScript `number` variable and not lose any digits.
...
Internally, the function uses PostgreSQL built-in [gen_random_uuid()](https://www.postgresql.org/docs/current/functions-uuid.html),
